...
> **Core LLM Engine:** Google Gemini 3.5 Flash (`gemini-3.5-flash` via Google GenAI SDK)  
...
> **Music & Leitmotif Engine:** Google Lyria (Harmonic Soundtrack Composition & Synthesizer)  
> **Agent Framework:** Google Agent Development Kit (`google-adk`)  
> **Database & Memory:** ClickHouse Cloud (OLAP Archival Vault)  
...
1. **Emotional Film Curation:** Real-time semantic discovery across 800,000+ movies on TMDB with multi-route intelligence for auteur directors (*Denis Villeneuve, Alfred Hitchcock*), studios (*Studio Ghibli, A24*), decades (*80s, 90s*), and international themes.
2. **Musicological Soundtrack Breakdown:** Deep OST analysis extracting the composer, atmospheric vibe, and key tracks.
...
7. **AI Alignment & Multi-Constraint Fidelity Matrix (`Alt+A` / `Option+A`):** An interactive benchmark dashboard evaluating how accurately recommendations honor multiple simultaneous constraints (*emotional mood vs explicit director/theme/decade/dietary directives*) across 10 recent sessions with Pareto arbitration notes, KPI summary cards, and 1-click exports (*CSV, JSON, Markdown*).
8. **3D Irregular Emotional Constellation & Google Lyria Leitmotif Engine (*Cosmic Cinema Cartography*):** An interactive deep-space celestial asterism where all watched films in your history are plotted as radiant star-shaped nodes with organic beam connections. Features **$Z$-Depth cosmic scaling** (recent films shine brightly in the foreground while older memories gently drift into deep space with reduced opacity and scale). Powered by **Google Lyria**, each star node synthesizes an **original 5-second cinematic leitmotif** (*Space Synth, Celesta Bell, Noir Piano, Orchestral Strings, Parisian Waltz*) rendered with real-time WebAudio cathedral convolution reverb, warm sub-bass, and a synchronized note timeline.
9. **Cinema Courier & Epistle Agent (Google Gemini Concierge):** Drafts and dispatches bespoke cinema night letters to user email with curator reflections, step-by-step concession recipes, acoustic setting tips, and streaming links.
10. **1-Click Instant Re-roll & AI Battery Safe Mode:** Immediate re-curation button for alternative films matching the exact mood without re-filling forms, featuring resilient zero-500 fallback and friendly AI battery recharge alerts.
...
```text
                    ...
                    │  (Multilingual NSFW, Violence & Cinema Guard)   │
                    └────────────────────────┬────────────────────────┘
                                             │ (Safe Request)
                                             ▼
                    ┌─────────────────────────────────────────────────┐
   ...
   - Age Rating Limit (G/PG)
        ...
        └────────────────────────────────────┬───────────────────────────────────┘
               ...
               ▼                                                           ▼
┌──────────────────────────────────────────────┐    ┌──────────────────────────────────────────────┐
│  AI Alignment & Constraint Fidelity Matrix   │    │ 3D Emotional Constellation (Google Lyria)    │
│  • Multi-Constraint Fidelity Score (94.2%)   │    │ • Irregular Celestial Asterism & Star Nodes  │
│  • Pareto Arbitration Analysis               │    │ • Z-Depth Cosmic Layering (>5 Watched Films) │
│  • 10-Session Historical Audit Table         │    │ • 5s Original Leitmotifs per Film Star       │
│  • 1-Click Exports (CSV, JSON, Markdown)     │    │ • WebAudio Reverb DSP & Note Progress Sync   │
└──────────────────────────────────────────────┘    └──────────────────────────────────────────────┘
                                             │
                                             ▼
                    ┌─────────────────────────────────────────────────┐
...
* **Soundtrack Composition & Ambient Pad:** Google Lyria (Harmonic Score & Leitmotif Architecture)
...
* **AI Alignment & Multi-Constraint Fidelity Matrix (`Alt+A`):**
  - **Fidelity Score**: Quantitative percentage measuring compliance with complex user criteria.
  - **Pareto Arbitration**: Evaluates how the Master Orchestrator resolves competing constraints (*e.g., when the user asks for "Stress Relief" + "Denis Villeneuve", balancing director style with emotional soothing*).
  - **Audit & Exports**: 10-session audit table with CSV, JSON, and Markdown export capabilities for evaluators and judges.
* **3D Irregular Emotional Constellation & Google Lyria Leitmotif Engine:**
  - **Longitudinal Cartography**: Synthesizes your watched movie history into an organic, irregular celestial asterism.
  - **$Z$-Depth Cosmic Layering**: Recent films shine in the foreground while older cinematic memories drift into deep space ($Z$-depth grading for $>5$ watched movies).
  - **Google Lyria Acoustic Synthesizer**: Generates an original 5-second cinematic leitmotif for each star node (*Space Synth, Celesta Bell, Noir Piano, Orchestral Strings, Parisian Waltz*) rendered with procedural cathedral convolution reverb, sub-bass support, and synchronized note timeline.
...
* **Multilingual Responsible AI Content Guardrail:** Filters NSFW, gore, and explicit violence in multiple languages while protecting cinema whitelists (*e.g. preventing false positives on Alfred Hitchcock*).
* **Resilient Zero-500 Safe Mode:** Automatically catches API quota limits and gracefully serves high-quality auteur alternatives with friendly in-page AI Power Nap notices.
